...
| `completeness` | `status`: `complete` \| `partial` \| `crashed`; `child_exit_code`; `module_errors`: array of `{rel, count}` | REQUIRED (`module_errors` OPTIONAL for compatibility). `module_errors` is the structured accounting for modules whose load/collection failed before any test could be observed (each entry a worktree-relative `rel` and its error `count`); it is what makes test loss legible where `observations` alone cannot (INV-2 accounting), not a place for raw error message text (message bodies belong to `observations`/`finding`, are unredacted free text, and are not digest material). Enumeration order MUST be deterministic (`rel` ascending). Adapters that can observe this capability SHOULD record it even when empty (`[]`). Records produced before this field existed omit it; consumers MUST NOT treat its absence as a hard error (§9.4). |
