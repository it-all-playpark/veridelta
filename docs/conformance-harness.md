...
| `{"do": "run", "id": "A", "args": ["math.test.ts"]?, "runner": "playwright"?, "expectExit": 0?, "expectReport": true?, "env": {"K":"V"}?}` | Invoke `vdelta run --report json -- <runner command> [args...]` in the workspace. `runner` selects which runner the step drives: omitted (default) invokes vitest (`node <vitest.mjs> run [args...]`); `"playwright"` is the only other accepted value and invokes playwright (`node <@playwright/test cli.js> test [args...]`). `args` are the runner's own CLI filters (selector). Stdout is parsed as a report JSON and stored under `id`; the recorded run is resolved via the report's `current.run_id`. `expectExit` checks the exit code (child passthrough). `expectReport: false` asserts the degraded path (stdout is NOT a report; raw child output passes through) — in that case no report is stored. The workspace config-boundary marker (§2) is written before a vitest run only; playwright's config search only ever looks at its cwd (no ancestor climb), so the marker is unnecessary and would be pure workspace pollution for a playwright run. |
| `{"do": "compare", "id": "cmp", "baseline": "A"?, "current": "B"?, "ref": "<git-ref>"?, "superset": true?, "expectExit": 0?, "assertDeterministic": true?}` | Invoke `vdelta compare` with `--report json`. With `baseline`+`current`: explicit-run-id mode using the run ids recorded by those steps. With `ref`: git-ref mode (`--ref`), current defaults to the last recorded run unless `current` is given. With neither: previous-comparable mode against the last run. With `superset`: previous-superset mode (`--superset`); current defaults to the last recorded run unless `current` is given; mutually exclusive with `ref` and with `baseline`+`current` (harness error, mirroring the CLI's usage error). Stdout report stored under `id`. `assertDeterministic` re-executes the identical command and byte-compares the two stdouts (§13.3). |
